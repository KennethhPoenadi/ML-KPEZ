...
from tubes2_ml.scratch.layers.recurrent import SimpleRNN


class ScratchRNNCaptioner:
    def __init__(
        self,
        feature_projection: Dense,
        embedding: Embedding,
        recurrent_layers: list[SimpleRNN],
        output_dense: Dense,
        injection_mode: str = "pre",
        context_fusion: Dense | None = None,
    ):
        if not recurrent_layers:
            raise ValueError("At least one SimpleRNN layer is required")
    ...
    @classmethod
    def from_keras_model(cls, keras_model) -> "ScratchRNNCaptioner":
        ...
        while True:
            try:
                keras_layer = keras_model.get_layer(f"rnn_{index}")
            except ValueError:
                break
            layer = SimpleRNN()
...
def build_scratch_rnn_captioner_from_keras(keras_model) -> ScratchRNNCaptioner:
    return ScratchRNNCaptioner.from_keras_model(keras_model)
